...
- `src/markdownItPlugin/**` — the wiki plugin contributed to the Markdown preview via `contributes["markdown.markdownItPlugins"]`. Rewrites `[[...]]` into navigable links and expands `![[...]]` embeds. VSCode tokenizes the preview by string, so a core rule cannot see which file is rendered (`env.currentDocument` is undefined at tokenize time); the previewed file's path is supplied by the extension (`getDocumentPath`, backed by `vscode.window.activeTextEditor`) and seeds the embed-cycle guard. **No `vscode` imports** (runs in the preview process).
